...
local breader = require("Vendor.bitreader")
local time = require("Vendor.time")
...
if not itemdata.ptrs:all(function(v)
	return v ~= nil
end) then
...
itemdata.parse_linkshell_name = function(data, offset)
	...
	return data:slice(offset)
		:split("")
		:map(string.zerofill - { 8 } .. math.binary .. string.byte)
		:flatten()
		:concat()
		:chunks(6)
...
itemdata.parse_signature = function(data, offset)
	...
	return data:slice(offset)
		...
		:chunks(6)
		:map(
			rawget + { ("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz{"):split("") }
				.. tonumber - { 2 }
		)
		:flatten()
		:concat()
...
itemdata.parse_raw_augments = function(item, ritem)
	...
	if data.augment_subkind >= 0 and bit.band(data.augment_subkind, 0x80) == 0 then
		...
		if bit.band(data.augment_subkind, 0x08) ~= 0 then
			...
			local divisor = switch(
				data.raw.craftmanship_kind,
				T({
					[1] = function()
						return 3000
					end,
...
itemdata.parse = function(item, is_equipped)
	...
	return switch(
		...
		T({
			[0x03] = function()
				return itemdata.parse_item_fish(item, ritem)
			end,
			[0x04] = function()
				return itemdata.parse_item_weapon(item, ritem, is_equipped)
			end,
			[0x05] = function()
				return itemdata.parse_item_armor(item, ritem, is_equipped)
			end,
			[0x06] = function()
				return itemdata.parse_item_linkshell(item, ritem)
			end,
			[0x0A] = function()
				return itemdata.parse_item_furnishing(item, ritem)
			end,
			[0x0C] = function()
				return itemdata.parse_item_flowerpot(item, ritem)
			end,
			[0x0E] = function()
				return itemdata.parse_item_mannequin(item, ritem)
			end,
			[0x0F] = function()
				return itemdata.parse_item_book(item, ritem)
			end,
			[0x11] = function()
				return itemdata.parse_item_betting_slip(item, ritem)
			end,
			[0x12] = function()
				return itemdata.parse_item_soul_plate(item, ritem)
			end,
			[0x13] = function()
				return itemdata.parse_item_reflector(item, ritem)
			end,
			[0x14] = function()
				return itemdata.parse_item_log(item, ritem)
			end,
			[0x15] = function()
				return itemdata.parse_item_lottery_ticket(item, ritem)
			end,
			[T({ 0x16, 0x17 })] = function()
				return itemdata.parse_item_tabula(item, ritem)
			end,
			[0x1A] = function()
				return itemdata.parse_item_evolith(item, ritem)
			end,
			[0x1F] = function()
				return itemdata.parse_item_crafting_set(item, ritem)
			end,
			[switch.default] = function()
				return itemdata.parse_item_default(item, ritem)
			end,
		})
	)
end
